...
import { initializeLocalStorage, getFromLocalStorage, saveToLocalStorage, LocalStorageKeys, generateId, findUserById, updateTodoInStorage, deleteTodoFromStorage } from "../utils/mockData";
...
export const getAPI = async (endpoint: string, token?: string) => {
   try {
      ...
      switch (endpoint) {
         ...
         default:
            // Handle single project endpoint: projects/{projectId}
            if (endpoint.startsWith("projects/") && endpoint.split("/").length === 2) {
               ...
               const allSections = getFromLocalStorage(LocalStorageKeys.SECTIONS) || [];
               const projectSections = allSections.filter((section: any) => section.project_id === projectId);
               
               // Get all todos for this project (using project ID)
               const allTodos = getFromLocalStorage(LocalStorageKeys.TODOS) || [];
               const projectTodos = allTodos.filter((todo: any) => todo.project === projectId);
               
               // Get all comments for these todos
               const allComments = getFromLocalStorage(LocalStorageKeys.COMMENTS) || [];
               
               // Enhance todos with comments and assigned users
               const enhancedTodos = projectTodos.map((todo: any) => {
                  const todoComments = allComments.filter((comment: any) => comment.todo === todo._id);
                  const assignedUsers = todo.assignedUsers 
                     ? todo.assignedUsers.map((userId: string) => findUserById(userId)).filter(Boolean) 
                     : [];
                  return {
                     ...todo,
               ...
               // Map sections to include their todos
               const mappedTodos = projectSections.map((section: any) => ({
                  id: section._id,
                  todos: todosBySection[section._id] || [],
               }));
